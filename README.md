...
```text
/plugin marketplace add withoneai/one-claude-plugin
/plugin install one@one
```

> Once approved in Anthropic's community marketplace, you'll also be able to run
...
Pick **One** → **Authenticate**. Your browser opens One's consent screen, where you choose
...
`execute_one_action` performs a **live** call (sending email, creating records, …), so Claude
Code asks you to approve it. The three discovery tools are read-only.
...
To change scoping later: `/mcp` → **One** → **Clear authentication**, then authenticate again.
...
- This plugin: https://github.com/withoneai/one-claude-plugin
